...
consort_diag<-function(consort_tbl){
  ...
  if ("Has_at_least_1_SCr" %in% consort_tbl$CNT_TYPE){
    tbl<-data.frame(CNT_TYPE=c("Initial",
                    ...
                    label_txt=c("Inpatient visit with LOS >= 2\nand of age >= 18",
                                ...
                                "Excluded: Less than 2 SCr Record",
                                "Excluded: Pre-existance of ESRD,\ndialysis and renal transplantation",
...
#   #assign("bp_fake_slp", bp_slp_eligb %>% filter(df > 1 & sd >= 1e-2) ,envir=.GlobalEnv)
        
    # bp_slp<-bp_slp_eligb %>%
      # filter(df > 1 & sd >= 1e-2) %>%
      # nest(c(timestamp,value,df,sd)) %>%
      # mutate(
